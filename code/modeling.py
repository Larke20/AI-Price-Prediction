...
def generate_models(df, start, end, epochs):
    ...
    for num_inputs in range(start, end + 1):
        generator = TimeseriesGenerator(inputs.values, targets.values, length=num_inputs, batch_size=32)
...
def make_prediction(best_model, input_data):
    prediction = best_model.predict(np.array(input_data), verbose=0)
...
def main_function(processed_df, df, start=1, end=3, epochs=10, user_path=get_path()):
    ...
    generator = TimeseriesGenerator(processed_df.drop(columns=['date']).values, np.zeros(len(processed_df)), length=num_inputs, batch_size=1)
    # val_predictions = []
    # for i in range(len(generator)):
    #     input_data = generator[i][0]
    #     # Make a prediction for the next day's close price
    #     next_day_prediction = best_model.predict(np.array(input_data), verbose=0)
    #     val_predictions.append(next_day_prediction[0][0])
    
    with ThreadPoolExecutor(max_workers=50) as executor:
        # Map the make_prediction function over the input data
        futures = [executor.submit(make_prediction, best_model, generator[i][0]) for i in range(len(generator))]
        # Collect results as they complete
        val_predictions = [future.result() for future in futures]

    h_max = df['close'].max()
    h_min = df['close'].min()
    # pred_prices = []
    # for i in val_predictions:
    #     pred_prices.append(i[0])
    # pred_prices = [price * (h_max - h_min) + h_min for price in pred_prices]
    pred_prices = [price * (h_max - h_min) + h_min for price in val_predictions]
    #print(f"predprices=   {pred_prices}")
    pred_prices = [i[0] for i in pred_prices]
    old_df = df
    df = df.iloc[:len(val_predictions)]
    
    trace1 = go.Scatter(
        x=df['date'].iloc[-len(pred_prices):],
        y=df['close'].iloc[-len(pred_prices):],
        mode='lines+markers',
        name='True Price',
    ...
    file_path = os.path.join(
        f'{user_path}AI-Price-Prediction/data/', 'true_vs_pred.html')
...
def main():
    ...
    user_path = get_path()
    df = pd.read_csv(f'{user_path}AI-Price-Prediction/data/processed_{ticker}.csv')
    og_df = pd.read_csv(f'{user_path}AI-Price-Prediction/data/{ticker}.csv')
